...
@Service
public class EncounterConditionServiceImpl implements EncounterConditionService {
	...
	@Override
	public EncounterCondition IdOrName(String idOrName) {
		String url = urlBase.concat(urlApi).concat(urlApiVersion).concat(urlApiEndpoint).concat(idOrName);
		return  restClient.getForObject(url, EncounterCondition.class);
	}
