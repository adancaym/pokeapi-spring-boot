...
@Service
public class MoveCategoryServiceImpl implements MoveCategoryService {
	...
	@Override
	public MoveCategory IdOrName(String idOrName) {
		String url = urlBase.concat(urlApi).concat(urlApiVersion).concat(urlApiEndpoint).concat(idOrName);
		return  restClient.getForObject(url, MoveCategory.class);
	}
