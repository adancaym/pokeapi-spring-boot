...
@Service
public class PokeathlonStatServiceImpl implements PokeathlonStatService {
	...
	@Override
	public PokeathlonStat IdOrName(String idOrName) {
		String url = urlBase.concat(urlApi).concat(urlApiVersion).concat(urlApiEndpoint).concat(idOrName);
		return  restClient.getForObject(url, PokeathlonStat.class);
	}
