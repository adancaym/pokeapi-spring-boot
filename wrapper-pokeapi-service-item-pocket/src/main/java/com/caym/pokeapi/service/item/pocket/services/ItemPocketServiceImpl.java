...
@Service
public class ItemPocketServiceImpl implements ItemPocketService {
	...
	@Override
	public ItemPocket IdOrName(String idOrName) {
		String url = urlBase.concat(urlApi).concat(urlApiVersion).concat(urlApiEndpoint).concat(idOrName);
		return  restClient.getForObject(url, ItemPocket.class);
	}
