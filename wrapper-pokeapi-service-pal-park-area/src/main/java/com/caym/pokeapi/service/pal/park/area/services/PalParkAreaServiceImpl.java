...
@Service
public class PalParkAreaServiceImpl implements PalParkAreaService {
	...
	@Override
	public PalParkArea IdOrName(String idOrName) {
		String url = urlBase.concat(urlApi).concat(urlApiVersion).concat(urlApiEndpoint).concat(idOrName);
		return  restClient.getForObject(url, PalParkArea.class);
	}
