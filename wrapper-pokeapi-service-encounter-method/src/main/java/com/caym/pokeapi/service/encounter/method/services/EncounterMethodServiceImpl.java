...
@Service
public class EncounterMethodServiceImpl implements EncounterMethodService {
	...
	@Override
	public EncounterMethod IdOrName(String idOrName) {
		String url = urlBase.concat(urlApi).concat(urlApiVersion).concat(urlApiEndpoint).concat(idOrName);
		return  restClient.getForObject(url, EncounterMethod.class);
	}
