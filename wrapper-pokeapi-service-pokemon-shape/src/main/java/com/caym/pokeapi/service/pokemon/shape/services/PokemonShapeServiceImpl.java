...
@Service
public class PokemonShapeServiceImpl implements PokemonShapeService {
	...
	@Override
	public PokemonShape IdOrName(String idOrName) {
		String url = urlBase.concat(urlApi).concat(urlApiVersion).concat(urlApiEndpoint).concat(idOrName);
		return  restClient.getForObject(url, PokemonShape.class);
	}
