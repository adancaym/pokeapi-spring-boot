...
@Service
public class MoveTargetServiceImpl implements MoveTargetService {
	...
	@Override
	public MoveTarget IdOrName(String idOrName) {
		String url = urlBase.concat(urlApi).concat(urlApiVersion).concat(urlApiEndpoint).concat(idOrName);
		return  restClient.getForObject(url, MoveTarget.class);
	}
