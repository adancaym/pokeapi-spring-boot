...
@Service
public class ItemFlingEffectServiceImpl implements ItemFlingEffectService {
	...
	@Override
	public ItemFlingEffect IdOrName(String idOrName) {
		String url = urlBase.concat(urlApi).concat(urlApiVersion).concat(urlApiEndpoint).concat(idOrName);
		return  restClient.getForObject(url, ItemFlingEffect.class);
	}
