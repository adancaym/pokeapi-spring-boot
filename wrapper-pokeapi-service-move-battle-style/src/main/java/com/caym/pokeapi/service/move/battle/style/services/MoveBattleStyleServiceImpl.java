...
@Service
public class MoveBattleStyleServiceImpl implements MoveBattleStyleService {
	...
	@Override
	public MoveBattleStyle IdOrName(String idOrName) {
		String url = urlBase.concat(urlApi).concat(urlApiVersion).concat(urlApiEndpoint).concat(idOrName);
		return  restClient.getForObject(url, MoveBattleStyle.class);
	}
