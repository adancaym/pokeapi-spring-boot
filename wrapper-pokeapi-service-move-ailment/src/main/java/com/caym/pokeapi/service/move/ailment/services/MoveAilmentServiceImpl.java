...
@Service
public class MoveAilmentServiceImpl implements MoveAilmentService {
	...
	@Override
	public MoveAilment IdOrName(String idOrName) {
		String url = urlBase.concat(urlApi).concat(urlApiVersion).concat(urlApiEndpoint).concat(idOrName);
		return  restClient.getForObject(url, MoveAilment.class);
	}
