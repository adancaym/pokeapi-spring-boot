...
@Service
public class EncounterConditionValueServiceImpl implements EncounterConditionValueService {
	...
	@Override
	public EncounterConditionValue IdOrName(String idOrName) {
		String url = urlBase.concat(urlApi).concat(urlApiVersion).concat(urlApiEndpoint).concat(idOrName);
		return  restClient.getForObject(url, EncounterConditionValue.class);
	}
