...
@Service
public class PokemonColorServiceImpl implements PokemonColorService {
	...
	@Override
	public PokemonColor IdOrName(String idOrName) {
		String url = urlBase.concat(urlApi).concat(urlApiVersion).concat(urlApiEndpoint).concat(idOrName);
		return  restClient.getForObject(url, PokemonColor.class);
	}
